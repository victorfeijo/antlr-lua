...
from PandoraLexer import PandoraLexer
from PandoraParser import PandoraParser
from PandoraVisitor import PandoraVisitor
...
class CodeGenerator(PandoraVisitor):
    def __init__(self):
        self.types = {
            'Void': ir.VoidType(),
            'Int':  ir.IntType(32),
    ...
    #     return ir.Constan(tself.types['Int'], int(integer))


if __name__ == '__main__':
    inputs  = FileStream(sys.argv[1])
    lexer   = PandoraLexer(inputs)
    tokens  = CommonTokenStream(lexer)
    parser  = PandoraParser(tokens)
    tree    = parser.chunk()
